...
const storageForNewCakeImages = multer.diskStorage({
  destination(req, file, cb) {
...
const uploadForNewCakeImages = multer({ storage: storageForNewCakeImages });
...
router.post('/getcategorycakes', async (req, res) => {
  try {
    ...
    const categoryCakesWithImagesPromises = cakeToSend.map((cake) => CakeImage.find({ cake: cake._id })
      .then((result) => {
        cake.images = result;
        return cake;
      }));
...
router.post('/savenewcategory', upload.single('imageName'), async (req, res, next) => {
  try {
    ...
    const candidates = await CakeCategory.find();
    const imagesNamesArray = candidates.map((img, idx) => img.imageName);
...
router.post('/savenewcake', async (req, res) => {
  try {
    ...
    const cake = new Cake({
      ...
      fillings: req.body.cakeFillings.map((cakeFilling) => cakeFilling.id),
    });
    ...
    cake.save((err) => {
      ...
      if (fs.existsSync(newDir)) {
        ...
        imagesList.forEach((imageName) => {
          if (!req.body.cakeImages.map((cakeImage) => cakeImage.cakeImageName).includes(imageName)) {
    ...
    if (previousCake.name !== req.body.cakeName) {
      ...
      if (previousImagesCopy.length) {
        previousImagesCopy.forEach(async (previousImage) => {
          if (req.body.cakeImages.map((cakeImage) => cakeImage.cakeImageName).includes(previousImage.cakeImageName)) {
...
router.post('/getcakedata', async (req, res) => {
  try {
    ...
    const fillingsPromises = cake.fillings.map((fillingId) => Filling.find({ _id: fillingId })
